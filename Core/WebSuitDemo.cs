...
{
    [SuitAlias("H"), SuitInfo("hello command.")] public void Hello() { IO.WriteLine("Hello! MobileSuit!"); }

    [SuitAlias("cui")]
    public void CuiTest()
    {
        IO.Write("This Will Raise a Dead Lock. Only Async support.");
        var selected = IO.CuiSelectItemFrom
        (
            "Select one",
            new[] { "x", "y", "z" },
            null,
        ...
        if (yes)
        {
            var selecteds = IO.CuiSelectItemsFrom
            (
                "Select many",
                new[]
                {
    ...
    public static object NumberConvert(string arg) { return int.Parse(arg); }

    [SuitAlias("Sn")] public void ShowNumber(int i) { IO.WriteLine(i.ToString()); }
